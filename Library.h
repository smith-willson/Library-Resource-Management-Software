...
public:
    ...
    LibraryResource* findResourceByTitle(const string &title) const; // search by title ...
    bool canUserBorrow(User *user) const;                            // check borrow eligibility ...
    double calculateUserFines(User *user) const;                     // calculate total fines ...
    bool removeResource(LibraryResource *res);                       // remove resource if not borrowed before ...
    bool removeUser(User *user);                                     // remove user if no active borrow ...
